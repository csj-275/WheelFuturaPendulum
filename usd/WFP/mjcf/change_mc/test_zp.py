...
def dynamic_model(q, dq, tau):
    ...
    M = np.array([[2*J0, -2*J0*sin(alpha)*cos(q2), 2*J0*cos(alpha)],
                  [-2*J0*sin(alpha)*cos(q2), L0**2*mb-J0/4*(-5+cos(2*alpha)-2*cos(2*q2)*sin(alpha)**2)+mb*L1**2*sin(2*q2), cos(q2)*(mb*L0*L1-J0*sin(alpha)*cos(alpha))],
                  [2*J0*cos(alpha),cos(q2)*(mb*L0*L1-J0*sin(alpha)*cos(alpha)), (Ib+mb*L1**2+J0/2*(3+cos(2*alpha)))]])
    C = np.array([[0, b7*sin(q2)*dq2, 0],
                  [b7*sin(q2)*dq2, 0,0],
                  [-b7*sin(q2)*dq1, 0,0]])
    C = np.array([[0, b7*sin(q2)*dq2, 0],
                  [b7*sin(q2)*dq2, (mb*L1**2-J0*sin(alpha))*sin(2*q2)*dq2, -(J0*sin(alpha)*cos(alpha)-mb*L0*L1)*sin(q2)*dq2],
                  [-b7*sin(q2)*dq1, (mb*L1**2-b4)*sin(2*q2)*dq1/2, b6*sin(q2)*dq1]])
    G = np.array([[0],
                  [0],
                  [-mb*g*L1*sin(q2)]])
    F = np.array([[tau],
